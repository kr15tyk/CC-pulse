...
def send_alert_email(alerts: list[dict]) -> None:
      """Send an immediate alert email when keyword matches are found on a daily run.
          Uses a condensed format — the full weekly digest is still sent on Mondays.
              """
    if not alerts:
              return
    date_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    body = (
              ...
              f'<p style="margin:4px 0 0;font-size:0.85rem;opacity:0.85">{date_str} — immediate notification</p>'
              '</div>'
